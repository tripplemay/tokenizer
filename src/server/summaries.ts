...
async function getDeviceSummaryImpl(tenantId: string, range: RangeOption = "all") {
  ...
  return Array.from(ids).map((deviceId) => {
    ...
    return {
      ...
      lastHarnessSyncAt: device?.lastHarnessSyncAt?.toISOString() ?? null,
      harnessSyncStatus: device?.harnessSyncStatus ?? null,
      harnessDiagnostics: device?.harnessDiagnostics ?? null,
