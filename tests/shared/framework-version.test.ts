...
describe("清单与常量一致性", () => {
  it("从 framework manifest 派生发布清单，manifest 最新版本是 1.6.1", () => {
    expect(FRAMEWORK_RELEASES).toEqual(MANIFEST_RELEASE_VERSIONS);
    expect(MANIFEST_RELEASE_VERSIONS.at(-1)).toBe("1.6.1");
    expect(LATEST_FRAMEWORK_VERSION).toBe("1.6.1");
  });
...
describe("frameworkStanding 四种状态", () => {
  it("latest：与最新版一致", () => {
    expect(frameworkStanding("1.6.1")).toEqual({
      kind: "latest", behind: null, latest: "1.6.1"
    });
  });

  it("behind：1.6.0 比 manifest 最新 1.6.1 落后一版", () => {
    expect(frameworkStanding("1.6.0")).toEqual({
      kind: "behind", behind: 1, latest: "1.6.1"
    });
  });
  ...
  it("ahead：项目比服务端已知的最新还新时，不得谎报落后", () => {
    expect(frameworkStanding("9.9.9")).toEqual({
      kind: "ahead", behind: null, latest: "1.6.1"
    });
  });
