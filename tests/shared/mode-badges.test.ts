...
describe("ModeBadges framework release rendering", () => {
  it("renders the synced v1.5.3 project as latest without stale guidance", async () => {
  ...
  it("renders v1.5.2 as one release behind with sync guidance", async () => {
    const html = await renderModeBadges("1.5.2");

    expect(html).toContain("v1.5.2");
  ...
  it("keeps a future version as ahead without a misleading sync hint", async () => {
    ...
    expect(html).toContain("ahead:1.5.3");
    expect(html).not.toContain("behindN:");
    expect(html).not.toContain("syncHint");
  });
});
