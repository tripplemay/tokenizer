...
**本批次最终策略：** 当前发布中，Kimi 是唯一已验证、可注册的 external same-session bridge；Codex 保持已验证的 `local-cli` 路径。这个策略是当前 registry 的配置约束，不是 Tokenizer 或 Framework 中按工具名写死的白名单。未来 CLI 只有在完成同等强度的桥接验证并声明自己的 verified manifest 后，才会自动进入同一规则体系。
...
- Kimi Code ACP 可由 Harness 自有根会话通过原生 `Agent` / `AgentSwarm` 创建 child。桥接可用 nonce 绑定的 `tool_call` 和 completion 事件证明 child 的创建与完成，因此可以作为 external same-session bridge。
...
该 bridge 必须有 verified manifest、完整 protocol、固定 persona 映射，并继承 Kimi `local_cli` 的 verified adapter、sandbox 和 timeout。Codex integration 只保留 `local_cli`，不得声明 `subagent` object 或 legacy `subagent: true`。
...
Kimi receipt 只记录 session id、nonce 哈希/有限标识、child tool-call id、子代理类型和终态。只有 nonce、预期 Agent 类型和 completion 同时匹配时才接受 child；ACP 错误、权限请求、无 child 证据或 artifact 缺失都 fail-closed。session wire 只可作本机诊断，不能成为实现数据源或产物。
...
- 当前 registry 只把 Kimi 的 verified manifest 展开为 external `subagent` candidate；Codex 只发布 `local-cli`（以及独立配置的 A2A）candidate。
...
- runner 用 ACP 管理 Harness 自有 session，并强制 root 通过 `Agent` 委派一个带 nonce 的 child。
- 仅 nonce、类型和 completion 一致时接受 child；无证据、ACP 错误或权限请求都 fail-closed/准确映射。
- 不读取或泄露用户会话 wire、prompt、模型输出或凭据。
...
- Kimi bridge 在现有动态三角色选择器中自动出现并显示 external same-session provenance；Codex 只显示 local-cli 路径。
...
- 在已认证的本机 Kimi 上运行无源码写入 probe，得到合格的 parent-child provenance；Codex 只运行 local-cli 健康检查，不执行 child-bridge probe。
