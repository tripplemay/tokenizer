...
- **BL-SECURITY-P1：building（2026-08-10 起，8 features = 7 gen + 1 eval）**。前端评审 P1 安全簇六项（AUTH_SECRET 生产 fail-closed / callbackUrl 开放重定向 / tooltip 存储型 XSS / gate 决策 TOCTOU / enrollment 绑定 / quota accountKey）+ F-32 查询放大三件。**首次全程 v2 heterogeneous 派发**：generator=codex、evaluator=kimi、planner=Coordinator（intent da55b68a 已消费）。规划期两处核查改方案：F-04 的 `EnrollmentToken.usedById` 早已在场 → 零 schema/零协议/不 bump fv；F-01 若按报告用 `NODE_ENV` 判据会炸 Dockerfile builder（`next build` 内部即置 production）→ 改构建期信号。用户已确认生产 AUTH_SECRET 在位，F001 排 push 序**最后**（爆炸半径 = 全站 500）。spec：`docs/specs/BL-SECURITY-P1-spec.md`。
